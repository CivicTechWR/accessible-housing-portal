...
| Area                | Files                                                                                                                                      |
| ------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ |
| Public search page  | `app/listings/page.tsx`, `app/listings/listings.tsx`, `app/listings/query.ts`, `app/listings/data.ts`                                      |
...
5. Fetches public dynamic filter groups through `getListingsDashboardData`.
...
Public listing search accepts these query parameters through `listingQuerySchema`:
...
| `accessibility` | `true` or `false`, based on legacy and custom-field accessibility data.           |
...
## Dynamic Public Filters

Public filters come from `listing_field_definitions` records where:
...
- legacy `customFields.accessibilityFeatures` values using alias matching in `listing-feature-definitions.ts`
...
- `updateListingByIdService` checks edit access, merges `customFields`, updates property/listing columns, and syncs images.
...
`lib/listings/store.ts` maps form inputs into listing `customFields`.

Current custom field storage includes:

- structured `units`
- `amenities`
- legacy `accessibilityFeatures`
- canonical boolean field keys from `listing_field_definitions`
- application method and external application URL
- eligibility criteria
- property/building/unit metadata
- utilities and lease details

If a new listing field must be searchable or sortable at scale, prefer a normalized column. If it is project-configurable metadata, prefer `listing_field_definitions` plus `customFields`.
